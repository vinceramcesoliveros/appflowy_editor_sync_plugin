...
use super::operations::{block_ops::BlockOperations, update_ops::UpdateOperations};

use crate::doc::constants::{BLOCKS, DEFAULT_PARENT, ROOT_ID};
use crate::doc::document_types::{
    BlockActionDoc, BlockActionTypeDoc, CustomRustError, DocumentState,
};
...
impl DocumentService {
    ...
    pub fn init_empty_doc(&mut self) -> Result<Vec<u8>, CustomRustError> {
        ...
        log_info!(
            "init_empty_doc: Initializing blocks for doc_id: {}",
            self.doc_id
        );
    ...
    pub fn apply_action(
        ...
        // Process each action
        for block_action in actions {
            let blocks_map = root.get_or_init_map(&mut txn, BLOCKS);

            // Delegate to specialized operation handlers
            match block_action.action {
                BlockActionTypeDoc::Insert => {
                    BlockOperations::insert_node(&mut txn, blocks_map, block_action)?;
                }
                BlockActionTypeDoc::Update => {
                    BlockOperations::update_node(&mut txn, blocks_map, block_action)?;
                }
                BlockActionTypeDoc::Delete => {
                    let parent_id = block_action
                        .block
                        .parent_id
                        .unwrap_or_else(|| DEFAULT_PARENT.to_owned());

                    BlockOperations::delete_node(
                        &mut txn,
                        blocks_map,
                ...
                BlockActionTypeDoc::Move => {
                    if let (Some(old_path), Some(parent_id), Some(old_parent_id)) = (
                        ...
                        BlockOperations::move_block(
                            ...
                            old_parent_id,
                            &block_action.block.id,
                            block_action.block.prev_id,
                            block_action.block.next_id,
                        )?;
                    } else {
                        return Err(DocError::InvalidOperation(
                            "Missing required fields for move operation".into(),
                        )
                        .into());
                    }
                }
    ...
    pub fn apply_updates(&mut self, updates: Vec<Vec<u8>>) -> Result<(), CustomRustError> {
        ...
        log_info!(
            "apply_updates: Successfully applied updates for doc_id: {}",
            self.doc_id
        );
        Ok(result)
    }
