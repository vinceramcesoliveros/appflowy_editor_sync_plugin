...
// In other files
use crate::doc::constants::{
    ATTRIBUTES, BLOCKS, ID, NEXT_ID, PARENT_ID, PREV_ID, ROOT_ID, TEXT, TYPE,
};
...
impl UpdateOperations {
    ...
    pub fn apply_updates_inner(
        ...
        match Update::decode_v2(&merged_update) {
            ...
            Err(e) => {
                ...
                return Err(DocError::UpdateDecodingFailed(format!(
    ...
    pub fn extract_document_state<T: ReadTxn>(
        ...
        let blocks_map = match root.get(txn, BLOCKS) {
            ...
            _ => {
                return Err(DocError::StateError("Blocks map not found in document".into()).into())
            }
        };
        ...
        log_info!(
            "extract_document_state: Processing {} blocks",
            block_keys.len()
        );
        ...
        log_info!(
            "extract_document_state: Extracted {} blocks and {} parent-child relationships",
            blocks.len(),
            sorted_children.len()
        );
    ...
    fn extract_block<T: ReadTxn>(
        ...
        if let Some(yrs::Out::YMap(block_map)) = blocks_map.get(txn, id) {
                ...
                match DeltaOperations::deltas_to_json(txn, deltas) {
                    Ok(json_deltas) => match serde_json::to_string(&json_deltas) {
                        ...
                        Err(e) => {
                            ...
                            return Err(DocError::StateEncodingFailed(format!(
                                "Failed to serialize deltas: {}",
                                e
                            ))
                            .into());
                        }
            ...
            let block = BlockDoc {
                ...
                delta: delta_string,
                parent_id: block_map.get(txn, PARENT_ID).map(|out| out.to_string(txn)),
                prev_id: block_map.get(txn, PREV_ID).map(|out| out.to_string(txn)),
                old_parent_id: None,
                next_id: block_map.get(txn, NEXT_ID).map(|out| out.to_string(txn)),
            };
