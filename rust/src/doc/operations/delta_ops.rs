...
impl DeltaOperations {
    ...
    pub fn apply_delta_diff_to_text(
        ...
        log_info!(
            "apply_delta_diff_to_text: Starting with {} operations",
            delta.len()
        );
    ...
    fn parse_delta_operation(
        ...
        if d.contains_key(INSERT) {
            ...
            let insert = d.get(INSERT).and_then(|v| v.as_str()).ok_or_else(|| {
                DocError::InvalidOperation("Insert value must be a string".into())
            })?;
            ...
            *cursor_pos += insert_len;
            ...
            let retain =
                d.get(RETAIN).and_then(|v| v.as_u64()).ok_or_else(|| {
                    DocError::InvalidOperation("Retain value must be a number".into())
                })? as u32;
            ...
            *cursor_pos += retain;
            ...
            let delete =
                d.get(DELETE).and_then(|v| v.as_u64()).ok_or_else(|| {
                    DocError::InvalidOperation("Delete value must be a number".into())
                })? as u32;
    ...
    pub fn json_value_to_yrs_any(val: &Value) -> yrs::Any {
        match val {
            ...
            Value::Array(arr) => yrs::Any::Array(Arc::from(
                arr.iter()
                    .map(Self::json_value_to_yrs_any)
                    .collect::<Vec<_>>(),
            )),
            Value::Object(obj) => yrs::Any::Map(Arc::from(
                obj.iter()
...
mod tests {
    ...
    fn test_parse_delta_operation_insert() {
        ...
        let result =
            DeltaOperations::parse_delta_operation(&operation, &mut cursor_pos, &mut current_len);
